...
.. code:: ipython3
   ...
   path = r'C:\Users\ema3\Documents\MATS\MATS\Examples\Synthetic_Spectra'
   os.chdir(path)
...
When simulating spectra, the first step is to read in the reference line list.  This file can be generated following the :ref:`Generating Parameter Line lists`. The following code reads in the reference line list as a pandas dataframe, then resets the working directory.  
...
.. code:: ipython3   
   ...
   spec_1 =  simulate_spectrum(PARAM_LINELIST, wavenumbers = wavenumbers, wave_error = wave_error, 
...
The procedure for analysis for both simulating and loading spectrum are the same as described in :ref:`Fitting Experimental Spectra`. The next step is to combine all desired :py:class:`Spectrum` objects into a :py:class:`Dataset` object, where we give the dataset a name and specify the parameter line list to use for analysis/
