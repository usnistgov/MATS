...
def beta_jit(GammaD, NuOptRe, alpha):
    """ JIT-compiled beta correction (unchanged) """
    if alpha < 5.0:
...
def mHTprofile_vector_numba(nu0, GammaD, Gamma0, Gamma2, Delta0, Delta2, 
    ...
    If 'out' is provided, writes results into it to avoid memory allocation.
    """
    n_points = len(nu_grid)
    
    # Use provided buffer if available, otherwise allocate (legacy behavior)
    if out is None:
        result = np.zeros(n_points, dtype=np.float64)
    else:
        result = out

    # --- STANDARD mHTP PHYSICS ---
    ...
    if abs_c2 > 1.0e-9:
        # Pre-calc constants for SD loop
        ...
        vecabsX = 0.0; inv_c2_real = 0.0; inv_c2_imag = 0.0
        Y_real = 0.0; Y_imag = 0.0; abs_Y = 0.0
        csqY_real = 0.0; csqY_imag = 0.0

    # Main Loop (Scalar logic)
    for i in range(n_points):
        nu = nu_grid[i]
        diff_val = nu0 - nu
        
        val_real = 0.0; val_imag = 0.0

        if abs_c2 > 1.0e-9:
            num_real = c0_real
            num_imag = c0_imag + diff_val
            ...
            if abs_Y > vecabsX * 1.0e-15:
                ...
                if vecabsX > abs_Y * 3.0e-8:
                    z1_real = z2_real - 2.0 * csqY_real
                    z1_imag = z2_imag - 2.0 * csqY_imag
                else:
                    z1_real = c0_real * inv_nuD
                    z1_imag = (c0_imag + diff_val) * inv_nuD
                
                w1 = cpf(-z1_imag, z1_real)
                w2 = cpf(-z2_imag, z2_real)
            ...
            z_imag = (c0_imag + diff_val) * inv_nuD
            w = cpf(-z_imag, z_real)
            pre = 1.772453850905516 * inv_nuD
            A_real = pre * w.real
            A_imag = pre * w.imag
