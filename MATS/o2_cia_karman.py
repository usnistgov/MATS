...
class O2_CIA_Karman_Model:
    def __init__(self, band = 'singlet_delta'):
    ...
    def calculate_cia(self, wavenumbers, T, P,Diluent,
        ...
        model_wavenumber = self.df_Karman['Wavenumber (cm-1)'].values
        SO = (self.df_Karman['SO Temp Dep - d'].values*(T-296)**3 +self.df_Karman['SO Temp Dep - c'].values*(T-296)**2 + self.df_Karman['SO Temp Dep - b'].values*(T-296) + self.df_Karman['SO Temp Dep - a'].values)*self.df_Karman['Spin Orbit (cm-1 amagat-2) Normalized'].values    
        EXCH = (self.df_Karman['EXCH Temp Dep - d'].values*(T-296)**3 +self.df_Karman['EXCH Temp Dep - c'].values*(T-296)**2 + self.df_Karman['EXCH Temp Dep - b'].values*(T-296) + self.df_Karman['EXCH Temp Dep - a'].values)*self.df_Karman['Exchange (cm-1 amagat-2) Normalized'].values
