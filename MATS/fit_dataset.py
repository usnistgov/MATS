            ...
            for segment in list(set(spectrum.segments)):
                ...
                if spectrum.low_OD_regime:
                    ...
                    if spectrum.transmittance_space:
                        fit_coef = np.exp(-fit_coef*float(params['pathlength_' + str(spectrum_number) +'_'+ str(segment)]))
                        if self.dataset.CIA_model['model'] == "Karman":
                            simulated_cia = np.exp(-CIA[np.min(indices_segments[segment]): np.max(indices_segments[segment])+1]*float(params['pathlength_' + str(spectrum_number)+'_' + str(segment)]))
                        else:
                            simulated_cia = CIA[np.min(indices_segments[segment]): np.max(indices_segments[segment])+1]
                    else:
                        fit_coef = 1 - np.exp(-fit_coef*float(params['pathlength_' + str(spectrum_number)+'_'+ str(segment)]))
                        if self.dataset.CIA_model['model'] == "Karman":
                            simulated_cia = 1- np.exp(-CIA[np.min(indices_segments[segment]): np.max(indices_segments[segment])+1]*float(params['pathlength_' + str(spectrum_number)+'_' + str(segment)]))
                        else:
                            simulated_cia = CIA[np.min(indices_segments[segment]): np.max(indices_segments[segment])+1]
                    ...
                    if not spectrum.low_OD_regime:
                        ...
                        for segment in list(set(spectrum.segments)):
                            if spectrum.transmittance_space:
                                ...
                                CIA[np.min(indices_segments[segment]): np.max(indices_segments[segment])+1] = np.exp(-CIA[np.min(indices_segments[segment]): np.max(indices_segments[segment])+1]*float(result.params['pathlength_' + str(spectrum.spectrum_number) + str(segment)]))
                            else:
                                CIA[np.min(indices_segments[segment]): np.max(indices_segments[segment])+1] = 1- np.exp(-CIA[np.min(indices_segments[segment]): np.max(indices_segments[segment])+1]*float(result.params['pathlength_' + str(spectrum.spectrum_number) + str(segment)]))
